...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		.hasAnyRole("USER","ADMIN").antMatchers("/users").anonymous().antMatchers(HttpMethod.GET,"/menu-items").permitAll()
		.antMatchers(HttpMethod.PUT,"/menu-items").permitAll().antMatchers("/carts/**").permitAll()
	...
	public InMemoryUserDetailsManager inMemoryUserDetailsManager() {
		...
		userDetailsList.add(User.withUsername("user").password(passwordEncoder().encode("pwd"))
				.roles("USER").build());
		userDetailsList.add(User.withUsername("admin").password(passwordEncoder().encode("pwd"))
				.roles("ADMIN").build());
		LOGGER.info("End");
